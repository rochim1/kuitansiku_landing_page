    ...
    <div class="col-md">
        ...
        <div class="card">
            <div class="card-body">
                ...
                @else
                    <form action="{{ url('admin/simpanan') }}" method="POST" autocomplete="off">
                        @csrf
                        <div class="mb-3">
                            <label class="form-label">Judul<span class="text-danger fs-5">*</span></label>
                            <select name="id_katesim" class="form-control @error('id_katesim') is-invalid @enderror">
                                <option value="">pilih</option>
                                @foreach ($katesim as $item)
                                <option value="{{$item->id_katesim}}">{{$item->nama_katesim}}</option>
                                @endforeach
                            </select>
                            @error('id_katesim')
                        ...
                        <div class="mb-3">
                            ...
                            <textarea id="ck_editor_simpanan" name="deskripsi_simpanan">{{ old('deskripsi_simpanan') }}</textarea>
