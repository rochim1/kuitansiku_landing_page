...
@section('content')
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        ...
        <a href="{{ url('/admin/kegiatan_zis/create') }}" class="btn btn-primary btn-icon-split">
                            ...
                            @foreach ($kegiatanzis as $item)
                                <tr>
                                    ...
                                    <td class="text-center">
                                        @if (Storage::exists($item->gambar_kegiatanzis))
                                            <img src="{{ asset('storage/' . $item->gambar_kegiatanzis) }}"
                                                class="img-fluid rounded-start border border-4" alt="gambar kegiatanzis"
                                    ...
                                    <td class="width-table">
                                        <form id="myForm" class="d-inline" action="{{ url('/admin/kegiatan_zis/' . Crypt::encryptString($item->id_kegiatanzis)) }}" method="POST" >
                                            @method('DELETE')
                                            @csrf
