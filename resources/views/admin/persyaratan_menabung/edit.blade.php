...
<div class="row">
    <div class="col-md">
        ...
        <div class="card">
            <div class="card-body">
                @if (!$persyaratan_menabung)
                    <h6 class="text-center text-danger">Sistem Dalam Perbaikan, Silahkan Hubungi Developer!</h6>
                @else
                    <form action="{{ url('admin/persyaratan_menabung/' . Crypt::encryptString($persyaratan_menabung->id_persyaratan_menabung)) }}" method="POST" autocomplete="off">
                        @method('PUT')
                        @csrf
                        <div class="mb-3">
                            <label class="form-label">Persyaratan<span class="text-danger fs-5">*</span></label><br>
                            <input type="hidden" id="deskripsi_persyaratan_menabung" name="deskripsi_persyaratan_menabung" value="{{ old('deskripsi_persyaratan_menabung', $persyaratan_menabung->deskripsi_persyaratan_menabung) }}">
